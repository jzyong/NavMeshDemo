...
{
    /// <summary>
    /// 导出navmesh寻路层，服务器才用自定义json格式，详情参考：https://github.com/jzyong/GameAi4j
    /// </summary>
    public class PolygonNavMeshWindow : EditorWindow
    {
        ...
        [MenuItem("工具/导出服务器NavMesh")]
        static void ShowToolWindow()
        {
            Rect rect = new Rect(0, 0, 500, 400);
            PolygonNavMeshWindow window = (PolygonNavMeshWindow)EditorWindow.GetWindowWithRect(typeof(PolygonNavMeshWindow), rect, true, "服务器NavMesh数据导出");
            window.Show();
        }
        ...
        private void OnGUI()
        {
            ...
            {
                if (SceneManager.GetActiveScene().name.Contains(mapID.ToString()))
                {
                    CreatePolyNavMesh();
                }
            }
